...
export default function Updateavator({ avatar, setImage, ...rest }) {
  ...
  const updating = async () => {
    ...
    try {
      var img = await axios.post(
        "https://david-cors-anywhere.herokuapp.com/https://api.cloudinary.com/v1_1/dwu20/image/upload",
        avatardata
      );
      console.log(img);
  ...
  return (
    <div>
      <Button
        startIcon={
          <Avatar className="private_avatar" src={avatarr ? avatarr : avatar} />
        }
        color="primary"
